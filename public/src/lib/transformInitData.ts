...
export const getInitRCStatus = (mhv4Response: MHV4Response): boolean => mhv4Response.is_rc;
export const getInitProgStatus = (mhv4Response: MHV4Response): boolean => mhv4Response.is_progress;

function getInitMHV4Data<T>(
  mhv4Response: MHV4Response,
  key: keyof MHV4Data
): (number | boolean)[][] {
  return mhv4Response.mhv4_data_array.reduce((acc: (number | boolean)[][], mod, index) => {
    if ((index + 1) % 4 !== 0) {
